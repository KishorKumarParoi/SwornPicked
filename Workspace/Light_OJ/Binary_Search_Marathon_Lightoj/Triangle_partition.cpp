...
double Area_proportion(double ad,double ab, double ac,double bc)
{
    double s,S,ae,de;
    S= (ab+bc+ac)/2;
    ...
    double area_of_ABC = sqrt(S*(S-ab)*(S-ac)*(S-bc)) ;
    double area_of_ADE = sqrt(s*(s-ad)*(s-ae)*(s-de));

    double trapiarea = (area_of_ABC - area_of_ADE) ;
    double R = area_of_ADE / trapiarea ;
...
double calculation_of_AD(double ab,double ac,double bc,double R)
{
    ...
    double lo=0.0,hi=ab,mid,ad;

    for(int i=1; i<=100; i++)
    {
        mid = (lo+hi)/2.0;
        ad = mid;
        if(Area_proportion(ad,ab,ac,bc) < R)
            lo = mid;
        else
            hi = mid;
    }
